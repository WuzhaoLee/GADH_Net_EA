...
LABELS_OBJ = {2:"Ground", 5: "High Vegetation", 6: "Building"}
LABELS = sorted(LABELS_OBJ.keys())
LABEL_INDEXES = dict([(label, index) for index, label in enumerate(sorted(LABELS_OBJ.keys()))])
Map_label={2:0,5:2,6:2,7:1,8:1, 0:3,1:3,3:3,4:3}  # if predicted labels are 0,1,3,4, they are all others~
...
def generate_confusion_matrix(zipped):
    ...
    invalid_labels = []
    dim = len(LABELS)  # 3
    matrix = np.zeros((dim, dim+1), np.uint)  # 3 by 4
    skipped_count = 0
    for truth_val, prediction_val in zipped:
        ...
        prediction_index = Map_label.get(prediction_val)

        # if a label is found, but not tracked by this software, log it
        if truth_index is None:
            skipped_count += 1
            if truth_val == NULL_CLASS:
               continue  # This was junk, can't reliably score it; # don't go on
            if truth_val not in invalid_labels:
                print("Invalid truth LABEL: {}".format(truth_val))    #  9,17 tracking or not ???
