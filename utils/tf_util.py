  ...
  with tf.variable_scope(scope) as sc:
    ...
    if bn:
      outputs = batch_norm_for_conv1d(outputs, is_training,
                                      ...
                                      data_format=data_format)
      #outputs = sync_batch_norm_for_conv1d(outputs, is_training,
            #                          bn_decay=bn_decay, scope='bn')
  ...
  with tf.variable_scope(scope) as sc:
      ...
      if bn:
        outputs = batch_norm_for_conv2d(outputs, is_training,
                                       bn_decay=bn_decay, scope='bn',
                                      data_format=data_format)
        #outputs = sync_batch_norm_for_conv2d(outputs, is_training,
         #                           bn_decay=bn_decay, scope='bn')
...
def sync_batch_norm_for_conv1d(inputs, is_training, bn_decay, scope):
    """ Batch normalization on 1D convolutional maps. written by lwz 2019/02/28
    ...
    Args:
        ...
        bn_decay:    float or float tensor variable, controling moving average weight
        scope:       string, variable scope
        data_format: 'NHWC' or 'NCHW' in default:NHWC
    Return:
        normed:      batch-normalized maps
    """
    return sync_batch_norm(inputs, is_training, scope, [0, 1], bn_decay)


def sync_batch_norm_for_conv2d(inputs, is_training, bn_decay, scope):
    """ Batch normalization on 2D convolutional maps.written by lwz 2019/02/28
    ...
    Return:
        normed:      batch-normalized maps
    """
    return sync_batch_norm(inputs, is_training, scope, [0, 1, 2], bn_decay)
...
def sync_batch_norm(inputs,
    ...
    with tf.variable_scope(scope,'BatchNorm',reuse=reuse):
        ...
        gamma = tf.get_variable(name='gamma', shape=[num_outputs], dtype=tf.float32,
                                initializer=tf.constant_initializer(1.0), trainable=is_trainable,
                                collections=variables_collections)

        beta = tf.get_variable(name='beta', shape=[num_outputs], dtype=tf.float32,
                               initializer=tf.constant_initializer(0.0), trainable=is_trainable,
                               collections=variables_collections)
        ...
        if is_training is not None and is_trainable is not None:
            ...
            if int(outputs.device[-1]) == 0:
                update_moving_mean_op = tf.assign(moving_mean, moving_mean * bn_decay + mean * (1 - bn_decay))
                update_moving_var_op = tf.assign(moving_var, moving_var * bn_decay + var * (1 - bn_decay))
